...
def bound_theta(parameters):
    """
    Clips parameters to predefined bounds, works in transformed space to prevent nan or infs when operating on boundary-violating params
    ensures that during iterative updates (e.g. iterate_theta_vec), you don't wander into an invalid parameter region (like negative distance) 
    for the mean that we're finding
    """
...
def convert_to_ripple_params(params):
    """
    Converts Fisher-space parameters to physical waveform parameters used by the waveform generator (Ripple)
    remove transofmration prefixes, and computes derived parameters
    maps to the key names expected by Ripple
    allows FisherSamples.get_signal_in_detector to feed parameters into the waveform function
    """
    converted = dict()
    temp = params.copy()
...
def convert_to_fisher_params(params, fisher_params):
    """
    Converts physical parameters => Fisher parameters:
    """
    converted = dict()
...
def inner(a, b, psd_array, freqs):
    """
    inner product used by fisher - shouldn't we be dividing by T here somewhere??
    """
...
def optimal_snr(signal, psd, freqs):
    """
    maximum possible signal-to-noise ratio you could get if you matched your template perfectly to the true signal using matched filtering
    """
...
def draw_samples(mu, fisher, names, key, N=1000):
    """
    Draws Gaussian samples in Fisher parameter space
    Returns a dict of parameter samples keyed by parameter names
    """
...
def convert_to_physical(samples, rho0=None, dL0=None, generate=['component_masses', 'chi_eff', 'luminosity_distance', 'cos_iota']):
    """
    Applies inverse_operations to remove log_, logit_, etc. Can compute masses from chirp mass and mass ratio, effective spin, Distance from SNR 
    scaling etc
    """
    if generate is None:
        generate=[]
...
def filter_samples(samples):
    """
    helper: Removes invalid samples based on BOUNDS
    Converts samples to physical space before checking
    for post-processing: after you've generated your samples around the mean, clip the ones that are unphysical
    """
...
def compute_distance(v1, v2, g):
    """
    Computes the Fisher metric distance between two parameter vectors
    used in iterate_theta_vec to decide if updates have converged
    """
    delta = v2 - v1
