...
run families.  The redshift evolution supports BOTH a pure power law and the
Madau-Dickinson form, dispatched on what the prior supplies (no user-facing
flag): a prior with neither ``kappa`` nor ``dkappa`` selects
dN/dV/dt propto (1 + z)^lam (``LogDNDVPowerLaw``, defined here, since the
PISN model has no use for it), while a prior supplying ``dkappa`` (or
``kappa``) together with ``zp`` selects the shared
``intensity_models_fast.LogDNDV`` the PISN model uses, unchanged.  The
dispatch is a STATIC Python branch on ``kappa is None``, like ``pairing``.
Everything else -- cosmology, detector->source conversion, selection
...
class LogDNDVPowerLaw(object):
    ...
    Deliberately local to this module: it is the drop-in alternative to
    ``intensity_models_fast.LogDNDV`` (the Madau-Dickinson form, which the
    PISN model keeps and which this module still selects when the prior
    supplies kappa/zp), with the same interface and conventions --
    ...
    * ``from_log1p(log1p_z)`` for callers that already hold ``log(1 + z)``
    ...
    as a power law all the way to ``zmax``.  It is the LogDNDV limit
    kappa -> 0 up to the (reference-cancelling) constant log 2.
    """
    lam: object
    zref: object = 0.001
    zmax: object = 20
...
class LogDNDMDQDV_LVK(object):
    ...
    In both pairings the redshift factor log dNdV(z) is normalized to 1 at
    ``zref`` and is selected statically by ``kappa``: ``kappa is None`` (the
    default) gives the pure power law (1 + z)^lam (``LogDNDVPowerLaw``), and a
    non-None ``kappa`` -- which then requires ``zp`` -- gives the shared
    Madau-Dickinson ``intensity_models_fast.LogDNDV``.
    """
    alpha_1: object
    alpha_2: object
    mbreak: object
    ...
    mmax: object
    delta_m: object
    # Madau-Dickinson turnover, OPTIONAL: both None (the default) selects the
    # pure power law.  They sit here, after the non-default fields, only
    # because dataclass ordering requires it.
    kappa: object = None
    zp: object = None
    mref: object = 30.0
    qref: object = 1.0
    ...
    def __post_init__(self):
        if self.pairing not in ("lvk", "mt"):
            raise ValueError(f"unknown pairing: {self.pairing!r} "
                             f"(known: 'lvk', 'mt')")
        # Static Python branch (never a traced jnp.where): the redshift form
        # is fixed by which parameters the prior supplies, exactly like the
        # `pairing` flag.  `is None` is safe against traced values -- a jax
        # tracer is never None.
        if self.kappa is None:
            if self.zp is not None:
                raise ValueError(
                    "LogDNDMDQDV_LVK: zp given without kappa.  zp is a "
                    "Madau-Dickinson parameter; either give kappa too (M-D "
                    "evolution) or drop zp (pure power law (1+z)^lam).")
            self.log_dndv = LogDNDVPowerLaw(self.lam, self.zref, zmax=self.zmax)
        else:
            if self.zp is None:
                raise ValueError(
                    "LogDNDMDQDV_LVK: kappa given without zp.  The "
                    "Madau-Dickinson evolution needs both (add a zp line to "
                    "the prior, or drop kappa/dkappa for the pure power law "
                    "(1+z)^lam).")
...
def get_deterministic_parameters(sample):
    ...
    The dkappa -> kappa rule stays conditional: a pure-power-law prior
    supplies neither, and the absence of kappa is what selects
    LogDNDVPowerLaw in LogDNDMDQDV_LVK."""
    out = {}
...
def build_population_model(sample, pairing="lvk", **_ignored):
    ...
    The redshift evolution is chosen by the prior, with no flag: ``kappa``
    (derived from ``dkappa`` by get_deterministic_parameters, or sampled
    directly) plus ``zp`` give Madau-Dickinson, and their absence -- .get(...)
    returning None -- gives the pure power law (1+z)^lam."""
    return LogDNDMDQDV_LVK(
        ...
        lam=sample['lam'],
        kappa=sample.get('kappa'), zp=sample.get('zp'),
