...
class ZPDF(object):
    def __init__(self, lam, kappa, zp, zmax, cosmo):
        ...
        # DON'T normalize - just use for icdf
        unnorm_pdf = self(self.zinterp)
        self.cdfinterp = sint.cumtrapz(unnorm_pdf, self.zinterp, initial=0)
        self.cdfinterp = self.cdfinterp / self.cdfinterp[-1]  # Normalize CDF to 1
    
    def __call__(self, z):
        # Return UNNORMALIZED density
        if self.cosmo == 'default':
            return (1+z)**self.lam / (1 + ((1+z)/(1+self.zp))**self.kappa) * Planck18.differential_comoving_volume(z).to(u.Gpc**3/u.sr).value / (1+z)
        else:
            return self.cosmo.dVCdz(z) / (1+z)
            #return (1+z)**self.lam / (1 + ((1+z)/(1+self.zp))**self.kappa) * self.cosmo.dVCdz(z) / (1+z)
