...
# JAX BATCHED VERSION?
def compute_snrs_batch(df, detectors=['H1'], sensitivity='aligo', fmin=20.0, fmax=2048.0, deltaf=.25, f_ref=20.0, use_antenna=True):
    """
    ...
    if 'gps_time' in df.keys():
        gps_time = jnp.array(df["gps_time"])
        gmst = None
    else:
        gmst=jnp.array(df["gmst"])
    # Derived quantities
    m2 = m1 * q
    m1d = m1 * (1 + z)
    m2d = m2 * (1 + z)
    ...
    def precompute_detectors():
        ...
        for det in detectors:
            ...
            for i in range(nevents):
                if gmst[i] is None:
                    gmst[i] = lal.GreenwichMeanSiderealTime(lal.LIGOTimeGPS(float(gps_time[i])))%(2*np.pi)        
                Fp, Fc = lal.ComputeDetAMResponse(det_obj.response, float(ra[i]), float(dec[i]), float(psi[i]), float(gmst[i]))
                
                dt = 0.#lal.TimeDelayFromEarthCenter(det_obj.location, float(ra[i]), float(dec[i]), float(gps_time[i]))
                Fp_list.append(Fp)
                Fc_list.append(Fc)
                dt_list.append(dt)
    ...
    def snr_single(theta, event_idx, use_antenna):
    ...
    snr_fn = jax.vmap(snr_single, in_axes=(0, 0, None))
    snrs = snr_fn(theta_ripple, jnp.arange(nevents), use_antenna)
