...
jid1=$(sbatch --parsable "${SCRIPT_DIR}/real_dat_call.sh")
echo "submitted gen_inj:     ${jid1}"
 
 
jid2=$(sbatch --parsable --dependency=afterok:${jid1} "${SCRIPT_DIR}/inf_call.sh")
echo "submitted run_inf:     ${jid2}  (after ${jid1})"
 
jid3=$(sbatch --parsable --dependency=afterok:${jid1} "${SCRIPT_DIR}/call_inf.sh")
echo "submitted run_inf:     ${jid3}  (after ${jid1})"
